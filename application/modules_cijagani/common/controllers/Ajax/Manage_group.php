...
defined('BASEPATH') OR exit('No direct script access allowed');
	class Manage_group extends CI_Controller {
		...
		public function __construct()
		{
			...
			$this->data["per"]=$this->custom->getRows("security_master");
		}
		public function add()
		{
			is_ajax();
		...
		public function save()
		{
			...
			if($post)
			{
				$per_data1=$this->custom->getRows("security_master");
				foreach ($per_data1 as $value) {
					$per[]=$value->name;
				}
				d($per);
				...
				redirect('staff_management/manage_group');
			}
			else
			{
				show_404();
