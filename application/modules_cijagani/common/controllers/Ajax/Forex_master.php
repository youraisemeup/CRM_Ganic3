...
defined('BASEPATH') OR exit('No direct script access allowed');
	class Forex_master extends CI_Controller {
		...
		public function __construct()
		{
			...
			$this->table="currency_master";
			$this->logged_id = $this->session->user_id;
			$this->view_path = 'common/ajax/forex_master/';
		}
		public function add()
		{
			is_ajax();
			$this->data['currency_options']=$this->custom->createDropdownSelect("currency",array('iso','iso','name'),"Currency",array("-"," "));
		...
		public function save()
		{
			...
			if($post)
			{
				...
				redirect('combo_tables/forex_master');
			}
			else
			{
				show_404();
		...
		function fetchData(){
			...
			if($row)
			{
				...
				$this->data['currency_options']=$this->custom->createDropdownSelect("currency",array('iso','iso','name'),"Currency",array("-"," "),'',array($row->currency_name));
			}	
		}
